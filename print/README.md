...
Source for `DME2100_Digital_Narratives_Fall2026.pdf`, matched to the Fall 2025
syllabus template. The pipeline is the same one used for the Hacking Geometry
syllabus, so both courses produce identical-looking documents.
...
The website uses the same Roboto and the same `#006FC0`, so the page and the
document read as one set.
